...
export class GestureEngine {
  ...
  async processFrame(videoElement) {
    ...
    try {
      ...
      const ruleBased = this.detectRuleBasedGesture(primaryHand, videoElement);
      if (ruleBased) {
  ...
  detectRuleBasedGesture(hand, videoElement) {
    if (!hand || !hand.keypoints || hand.keypoints.length < 21) return null;

    // 1. Hand Detection Confidence Guard (reject partial/blurry hands)
    if (hand.score && hand.score < 0.72) return null;

    const kp = hand.keypoints;
    const vw = videoElement ? videoElement.videoWidth || 640 : 640;
    const vh = videoElement ? videoElement.videoHeight || 480 : 480;

    // 2. In-Frame Boundary Check: Reject hands cut off at edges (within 4% boundary margin)
    const marginX = vw * 0.04;
    const marginY = vh * 0.04;

    const keypointsToCheck = [0, 4, 8, 12, 16, 20];
    const isCutOff = keypointsToCheck.some((idx) => {
      const pt = kp[idx];
      return !pt || pt.x < marginX || pt.x > (vw - marginX) || pt.y < marginY || pt.y > (vh - marginY);
    });
    ...
    // 3. Minimum Palm Scale Guard (reject partial palm keypoints)
    const palmScale = dist(0, 9);
    if (palmScale < 20) return null;
    const wrist = kp[0];
